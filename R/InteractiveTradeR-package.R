...
.onLoad <- function(libname, pkgname) {
  ...
  rprofile_path <- file.path(rprojroot::find_package_root_file(), ".Rprofile")
  ...
  params_saved    <- any(
    grepl(
      ...
      tryCatch(
        suppressWarnings(readLines(rprofile_path)),
        error = function(e){""}
      )
    )
  )
  ...
  if(save_defaults){
    ...
    usethis::ui_info(paste0("Creating .Rprofile: ", rprofile_path))
